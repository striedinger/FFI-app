    	...
    	if($company = $this->company->forId($id)){
			if($request->user()->id == $company->user_id){
				if($request->isMethod('post')){
    				...
    				return view('icai.create', ['company' => $company, "education" => $this->education, "sectors" => $this->sectors, "markets" => $this->markets, "activities" => $this->activities, "barriers" => $this->barriers, "sources" => $this->sources, "partners" => $this->partners]);
    			}
			}else{
    ...
    public function update(Request $request, $id){
        if($icai = $this->icai->forId($id)){
            ...
            return view('icai.update', [
                'icai' => $icai,
                "education" => $this->education, "sectors" => $this->sectors, "markets" => $this->markets, "activities" => $this->activities, "barriers" => $this->barriers, "sources" => $this->sources, "partners" => $this->partners
            ]);
        }else{
            abort(404);
        }
